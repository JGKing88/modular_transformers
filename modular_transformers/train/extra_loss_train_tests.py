        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                if loss_hooks is not None:
                    ...
                    if (
                        normalize_loss
                        and len(np.trim_zeros(logit_losses_list))
                        == normalization_batch_size
                    ):
                        logit_losses_list[step % normalization_batch_size] = (
                            logit_loss.item() * logit_multiplier
                        )
                        extra_losses_list[step % normalization_batch_size] = (
                            extra_loss.item() * inter_multiplier
                        )

                        logit_loss = logit_loss / np.mean(
                            np.trim_zeros(logit_losses_list)
                        )
                        extra_loss = extra_loss / np.mean(
                            np.trim_zeros(extra_losses_list)
                        )
