...
def evaluate():
    ...
    for step, batch in tqdm(enumerate(eval_dataloader), total=len(eval_dataloader)):
        with torch.no_grad():
            ...
            outputs = model(batch_pt[0].transpose(1, 0), labels=batch_pt[2].transpose(1, 0))
...
if __name__ == "__main__":
    ...
    model_name='gpt2'
    data='10M'
    train_config = {"lr": 0.0006, "num_epochs": 50, "correct_bias": True, "seed": 42, "batch_size": 64}
    ...
    if os.path.exists(os.path.join('/om/user/ehoseini/MyData/miniBERTa_v2/',f'miniBERTa-{data}-crunched',f'train_context_len_{CONTEXT_LENGTH}')):
        grouped_pad_train=load_from_disk(os.path.join('/om/user/ehoseini/MyData/miniBERTa_v2/',f'miniBERTa-{data}-crunched',f'train_context_len_{CONTEXT_LENGTH}'))
        ...
        """
        mini_dataset = load_dataset('/om/user/ehoseini/MyData/miniBERTa_v2/',
                                    f'miniBERTa-{data}')
        tokenized_datasets = mini_dataset.map(tokenize_function, batched=True, remove_columns=["text"])

        grouped_train = Group_Texts(tokenized_datasets["train"], tokenizer, seq_len=CONTEXT_LENGTH, padding=True, batch_size=500)
        grouped_test = Group_Texts(tokenized_datasets["test"], tokenizer, seq_len=CONTEXT_LENGTH, padding=True, batch_size=500)
        grouped_valid = Group_Texts(tokenized_datasets["validation"], tokenizer, seq_len=CONTEXT_LENGTH, padding=True, batch_size=500)

        # Call the grouping function with Group_Texts
        grouped_pad_train = grouped_train.group_texts()
        grouped_pad_train.save_to_disk(os.path.join('/om/user/ehoseini/MyData/miniBERTa_v2/',f'miniBERTa-{data}-crunched',f'train_context_len_{CONTEXT_LENGTH}'))
        print(f"Total number of tokens in training dataset: {grouped_pad_train.shape[0]*CONTEXT_LENGTH}")
        grouped_pad_test = grouped_test.group_texts()
        grouped_pad_test.save_to_disk(os.path.join('/om/user/ehoseini/MyData/miniBERTa_v2/', f'miniBERTa-{data}-crunched',
                                                f'test_context_len_{CONTEXT_LENGTH}'))
        print(f"Total number of tokens in test dataset: {grouped_pad_test.shape[0]*CONTEXT_LENGTH}")
        grouped_pad_valid = grouped_valid.group_texts()
        grouped_pad_valid.save_to_disk(os.path.join('/om/user/ehoseini/MyData/miniBERTa_v2/', f'miniBERTa-{data}-crunched',
                                               f'valid_context_len_{CONTEXT_LENGTH}'))

        print(f"Total number of tokens in validation dataset: {grouped_pad_valid.shape[0]*CONTEXT_LENGTH}")

    train_dataloader = DataLoader(grouped_pad_train, shuffle=True, batch_size=train_config['batch_size'])
    ...
    for epoch in tqdm(range(train_config['num_epochs'])):
        ...
        # Log validation loss and accuracy to wandb
                ...
                save_dir=Path(f'/om2/user/ehoseini/MyData/bplm/miniberta_{data}/distilgpt2/{run_name}/checkpoint_{abs_step}')
