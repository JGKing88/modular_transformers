...
def compute_model_activations(model, indexed_tokens, device):
    ...
    for i in tqdm(range(len(indexed_tokens))):
        ...
        with torch.no_grad():
            outputs = model(
                tokens_tensor, output_hidden_states=True, output_attentions=False
            )
...
def compute_model_curvature(all_layers):
    ...
    for idk, layer_act in tqdm(enumerate(all_layers)):
        ...
        for idy, vec in enumerate(sent_act):
            curve = [
                np.dot(vec[idx, :], vec[idx + 1, :]) for idx in range(vec.shape[0] - 1)
            ]
            curve = np.clip(curve, -1, 1)  # added by Jack King
    ...
    torch.cuda.empty_cache()
    return dict(
        curve=curve_, curve_change=curve_change, all_layer_curve_all=all_layer_curve_all
    )
