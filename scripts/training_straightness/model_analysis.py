...
fine_tuned_models = [
    "finetuned-warmup2-lr0.0003-100-layer9-l2_curvature-768x12",
    "finetuned-768x12",
    "warmup2-finetuned-lr0.0003-100-l2_curvature-768x12",
    "multi-warmup2-finetuned-lr0.0003-1-l2_curvature-768x12",
    "gpt2",
    "finetuned-epochs20-768x12",
    "finetuned-warmup2-lr0.0002-1-layer9-l2_curvature-epochs20-768x12",
    "finetuned-warmup2-lr0.0003-100-layer8-l2_curvature-epochs10-768x12",
    "finetuned-warmup2-lr0.0006--1-layer8-l2_curvature-epochs10-768x12",
]
fulltrained_models = [
    "warmup10-lr0.0003-1-layer9-l2_curvature-768x12",
    "warmup10-lr0.0003-1-l2_curvature-768x12",
    "warmup5-lr0.0006-1-l2_curvature-768x12",
    "768x12",
    "warmup10-lr0.0006-1-layer9-l2_curvature-epochs20-768x12",
    "warmup5-lr0.0006-1-layer8-l2_curvature-epochs10-768x12",
    "warmup20-lr0.0006-1-layer8-l2_curvature-epochs20-768x12",
    "warmup10-lr0.0006-1-layer8-l2_curvature-epochs10-768x12",
    "epochs20-768x12",
    "warmup10-lr0.0003-1-layer8-l2_curvature-epochs10-768x12",
    "warmup10-lr0.0006-1-layer8-l1_curvature-epochs10-768x12",
    "warmup10-lr0.0006-1-layer9-l2_curvature-epochs10-768x12",
    "warmup10-lr0.0006-0.1-layer8-l2_curvature-epochs10-768x12",
    "warmup10-lr0.0006--0.1-layer8-l2_curvature-epochs10-768x12",
    "warmup10-lr0.0006-1-layer8-l0_curvature-epochs10-768x12",
]

dir_path = "/om2/user/jackking/modular_transformers/scripts/training_straightness/data"

model_names = [
    "7-embd-0.5-epochs10-768x12",
    "768x12",
    ...
    "3-attn-0.5-epochs10-768x12",
    "3-attn-0.8-epochs10-768x12",
    "3-embd-0.8-epochs10-768x12",
    "3-embd-0.5-epochs10-768x12",
    "7-embd-0.8-epochs10-768x12",
    "7-attn-0.8-epochs10-768x12",
    "7-attn-0.8-epochs10-768x12",
    "warmup20-lr0.0003-0.1-layer7-l0_curvature-epochs10-768x12",
    "warmup10-lr0.0003-0.1-layer7-l0_curvature-epochs10-768x12",
]
...
model_names = [
    "gpt2",
    "5-attn-0.5-5-embd-0.5-6-attn-0.5-6-embd-0.5-epochs10-768x12",
    "warmup10-scaled-lr0.0003-1-layer5-sparsity-epochs10-768x12",
    "warmup10-scaled-lr0.0003-1-layer6-curvature-epochs10-768x12",
    "warmup10-scaled-lr0.0003-1-layer5-curvature-epochs10-768x12",
    "warmup10-scaled-lr0.0003-1-layer6-sparsity-epochs10-768x12",
    "warmup10-scaled-lr0.0003-0.1-layer6-curvature-epochs10-768x12",
    "epochs10-768x12",
    "warmup10-scaled-lr0.0003-0.01-layer6-curvature-epochs10-768x12",
    "warmup10-scaled-lr0.0003-0.1-layer6-sparsity-epochs10-768x12",
    "warmup10-scaled-lr0.0003-0.1-layer6-curvature-epochs10-768x12",
    "warmup10-scaled-lr0.0003-0.01-layer6-curvature-epochs10-768x12",
    "multi-warmup10-scaled-lr0.0003-0.01-layer6-curvature-epochs10-768x12",
    "multi-warmup10-scaled-lr0.0003-0.1-layer6-curvature-epochs10-768x12"
]
...
def compute_surprisals(
    ...
    perturbation_section=None,
):
    try:
        surprisal_dict = torch.load(
            f"{dir_path}/surprisal_dict.pt", map_location=torch.device("cpu")
        )
    except:
        surprisal_dict = {}
    ...
    for model_name in model_names:
            ...
            perturbation_hooks = {
                perturbation_layer: [
                    (
                        ...
                        "all",
                        random_perturbation_function(perturbation_size),
                    )
                ]
            }
        ...
        for batch in tqdm(dataloader):
            ...
            new_batch["attention_mask"] = torch.stack(
        ...
        surprisal_dict[perturbation_type][surprisal_type][model_name] = np.array(
            all_surprisals
        )
...
def compute_entropy_with_5000_sentences(
    model_names,
    perturbation_type,
    surprisal_type,
    perturbation_size=None,
    ...
    try:
        surprisal_dict = torch.load(
            f"{dir_path}/epoch_dict.pt", map_location=torch.device("cpu")
        )
    except:
        surprisal_dict = {}
    ...
    for model_name in model_names:
        ...
        for i in range(0, 5000, 100):
            ...
            for i in range(input.shape[0]):
                surprisals.append(
                    -1
                    * torch.sum(probs[i] * log_probs[i], dim=-1).detach().cpu().numpy()
                )
    ...
    with open(f"{dir_path}/epoch_dict.pt", "wb") as f:
        torch.save(surprisal_dict, f)


def compute_surprisals_with_5000_sentences(
    model_names,
    perturbation_type,
    surprisal_type,
    perturbation_size=None,
...
def compute_surprisals_with_context(
    model_names,
    prefixes,
    queries,
    perturbation_type,
    ...
    for model_name in tqdm(model_names):
        ...
        for prefix, query in tqdm(zip(prefixes, queries)):
            surprisals = model.conditional_score(
                prefix, query, reduction=lambda x: -x.sum(0)
            )
            all_surprisals.extend(surprisals)
        surprisal_dict[perturbation_type][surprisal_type][model_name] = np.array(
            all_surprisals
...
if __name__ == "__main__":
    ...
    surprisal_type = "5000sentences"
    compute_surprisals_with_5000_sentences(model_names, "none", surprisal_type)
    compute_entropy_with_5000_sentences(model_names, "none", surprisal_type)

    perturbation_type = "activation"
    for perturbation_layer in [3, 4, 5, 6, 7]:  # [8, 1, 4, 5, 9]:
        for perturbation_section in [
            "post_block"
        ]:  # [""mid_block", "before_mlp", "before_attn"]:
            for perturbation_size in [0.1, 0.5, 1, 2]:
                print(
                    f"{perturbation_layer}, {perturbation_section}, {perturbation_size}"
                )
                compute_surprisals_with_5000_sentences(
                    model_names,
                    perturbation_type,
                    surprisal_type,
                    perturbation_size,
                ...
                compute_entropy_with_5000_sentences(
                    model_names,
                    perturbation_type,
                    surprisal_type,
                    perturbation_size,
